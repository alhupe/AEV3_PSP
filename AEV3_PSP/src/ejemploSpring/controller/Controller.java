...
public class Controller {
	...
	String autoritzatsPath = "autoritzats.txt";


	/**
	 * Maneja las solicitudes de la API relacionadas con películas.
	 *
	 * @param id El ID de la película o "all" para obtener información sobre todas las películas.
	...
	/**
	 * Obtiene las reseñas de una película específica desde el archivo correspondiente.
	 *
	 * @param id          El ID de la película.
	 * @param rutaArchivo La ruta al archivo que contiene las reseñas.
	 * @return Lista de reseñas de la película.
	 */
	private List<String> obtenerResenyes(String id, String rutaArchivo) {
	    ...
	    try (BufferedReader br = new BufferedReader(new FileReader(rutaArchivo))) {
	        ...
	        while ((linea = br.readLine()) != null) {
	            if (!tituloEncontrado && linea.startsWith("Titulo: ")) {
	                tituloEncontrado = true;
	            } else if (linea.contains(": ")) {
	                String resenya = linea.trim();
	...
	private String construirJsonConResenyes(String id, String titulo, List<String> ressenyes) {
	    StringBuilder jsonBuilder = new StringBuilder();
	    jsonBuilder.append("{")
	               .append("\"id\": \"").append(id).append("\", ")
	               .append("\"titol\": \"").append(titulo).append("\", ")
	               .append("\"ressenyes\": [");

	    for (String resenya : ressenyes) {
	        jsonBuilder.append("\"").append(resenya).append("\", ");
	    }
	    if (!ressenyes.isEmpty()) {
	...
	/**
     * Construye una cadena JSON que contiene información sobre las películas.
     *
     * @param peliculas Lista de objetos Pelicula.
     * @return Cadena JSON que representa la lista de películas.
     */
    private String construirJson(List<Pelicula> peliculas) {
        StringBuilder jsonBuilder = new StringBuilder();

        jsonBuilder.append("{\"titols\": [");

        for (Pelicula pelicula : peliculas) {
            jsonBuilder.append("{")
                       .append("\"id\": \"").append(pelicula.getId()).append("\", ")
                       .append("\"titol\": \"").append(pelicula.getTitulo()).append("\"")
                       .append("}, ");
        }
	...
	ResponseEntity<String> novaRessenya(@RequestBody Map<String, String> requestBody) {
	    ...
	    if (usuariAutoritzat(nomUsuari)) {
	        ...
	        // Actualizar la lista de archivos después de agregar la nueva reseña
	        llistaFitxers = directorio.list(new FiltreExtensio(".txt"));
